...
# at top of your NetBuilder file, once:
MY_SURROGATE = surrogate.atan(alpha=2.0)
	...
	def build_snn(self, net_dict):
		...
		for key in net_dict:
			...
			if "layer" in key:
				...
				if first:
					use_bias = net_dict[key].get("bias", False)
		...
		for step in range(input_spikes.shape[0]):
			...
			for layer in self.layers:
				...
				if "fc" in layer:
					...
					# ---- Residual skip from fc0 to fc1 ----
					#if layer == "fc1" and "fc0" in cur:
					#	cur[layer] = cur[layer] + cur["fc0"]
